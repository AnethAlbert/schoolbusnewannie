...
import 'package:newschoolbusapp/services/Class_apiService.dart';
...
class ProfilePicDialogBoxStudent extends StatefulWidget {
  ...
  @override
  State<ProfilePicDialogBoxStudent> createState() => _ProfilePicDialogBoxStudentState();
}

class _ProfilePicDialogBoxStudentState extends State<ProfilePicDialogBoxStudent> {
  bool _isLoading = true;
  ...
  void _onPressed(BuildContext context) async {
    ...
    try {
      await apiService.addStudendTripList(Tripid, _id!,);
      getUserByFingerprint();
      print('Student ididid:::::::${_id}');
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        setState(() {
          ...
          stationName = stationDetails != null ? stationDetails.name ?? 'N/A' : 'N/A';
          _isLoading = false; // Set loading state to false
        });
      } else {
  ...
  Future<Station?> getStationDetails(int? stationId) async {
    ...
    try {
      ...
      List<Station>? stationList = await StationApiService().getStationById(stationId);
